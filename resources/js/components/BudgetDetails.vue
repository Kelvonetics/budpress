<template>
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header-left col-xl-9 col-md-7 col-sm-12 mb-2 breadcrumb-new">
          <h3 class="content-header-title mb-0 d-inline-block">Budget Imprests</h3> 
          <div class="row breadcrumbs-top d-inline-block">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Home</a>
                </li>
                <li class="breadcrumb-item active">Budgets - Imprest
                </li>
              </ol>
            </div>
          </div>
        ...
        <div class="content-header-right col-xl-1 col-md-2 col-sm-12">
          <div class="dropdown float-md-right">
            <button class="btn btn-primary round btn-glow px-2" type="button" data-animation="pulse" data-toggle="modal" data-target="#addImprestModal" @click="clearForm()">New</button>
          </div>
        </div>
      </div>
                      ...
                      <tbody>
                        <tr style="background: #fff;" v-for="imprest in filteredImprests" v-bind:key="imprest.id">
                            ...
                            <td>
                                ...
                                <a class="pull-right">
                                    <router-link class="" v-bind:to="'/budget-imprest-expenses/' + imprest.id" style="">
                                        <span class="la la-eye text-inverse m-r-10" title="View Imprest Expenses" style="margin :1px 2px; color:#6cb2eb"></span>
                                    </router-link>
                                </a>
                                ...
                                <span v-if="imprest.flow_stage_id === 2"> 
                                    <a class="pull-right" data-toggle="modal" data-target="#approveImprestModal" title="Review Imprest" 
                                        @click="reviewImprest(imprest)"><span class="la la-registered text-inverse m-r-10" style="margin :1px 2px; color: #396;"></span>
                                    </a> 
                                </span>
                                <span v-else-if="imprest.flow_stage_id === 3"> 
                                    <a class="pull-right" data-toggle="modal" data-target="#approveImprestModal" title="Approve Imprest" 
                                        @click="approveImprest(imprest)" id="approve"><span class="la la-check text-inverse m-r-10" style="margin :1px 2px; color: #396;"></span>
                                    </a> 
                                </span>
...
<script>
    export default
    {
        ...
        methods:{
            ...
            {
                let vm = this;
                page_url = page_url || '/api/budget-imprests?all=1'
                fetch(page_url)
                .then(res => res.json())
                .then(res => {
            ...
            {
                let vm = this;
                page_url = page_url || '/api/budget-imprests/' + this.$route.params.id,
                fetch(page_url)
                .then(res => res.json())
                .then(res => {
                    this.imprests = res.data;
            ...
            reviewImprest(imprest)
            {
                this.expense.actions=2;
                this.edit = true;
                this.imprest.id = imprest.id;
                this.imprest.imprest_id = imprest.id;
                this.imprest.name = imprest.name;
